...
| `ODR.md` | **Open Dynamic Range**, the project's own dynamics specification. **Normative**, versioned in its own header, and written so that somebody with no copy of this repository can implement it: every conformance case in its § 7 is a generated signal with a stated tolerance, and `packages/oaa_engine/test/conformance_test.dart` asserts each one. A change to the definition is a version bump and a row in its revision history, never a silent edit. Its Annex A is **informative** — what a reading means, not what it is — and may be revised without a bump. Its text is CC BY 4.0 so it can be reproduced. |
