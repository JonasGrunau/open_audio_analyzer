...
```sh
git clone https://github.com/JonasGrunau/open_music_analyzer
...
```sh
flutter run -d macos --dart-entrypoint-args --config-dir=/tmp/bel-scratch
...
```sh
open -a build/macos/Build/Products/Debug/bel.app --args --open-panel=shortcuts
```
...
AGPL-or-commercial. Nothing there may move into `engine/` or `bel_core/`, which
...
| Variable | For |
| --- | --- |
| `BEL_SIGNING_IDENTITY` | macOS Developer ID, e.g. `Developer ID Application: Name (TEAMID)` |
| `BEL_NOTARY_PROFILE` | A `xcrun notarytool store-credentials` profile |
| `BEL_WINDOWS_CERT` | Path to a `.pfx` |
| `BEL_WINDOWS_CERT_PASS` | Its password |
| `BEL_WINDOWS_PUBLISHER` | The certificate's subject, **exactly** — e.g. `CN=Jonas Grunau` |
...
- **A signed but un-notarised dmg is still refused by Gatekeeper.** The
  ...
  or neither.
- **`BEL_WINDOWS_PUBLISHER` must equal the certificate subject byte for byte.**
...
file; `packaging/icon/bel.svg` is its vector twin and carries the same numbers.
