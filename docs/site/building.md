...
| `OAA_ASC_NOTES_WAIT` | Optional. Seconds to wait for App Store Connect to finish processing the build, `900` by default, `0` to skip. A build is not a resource until it has processed, and What to Test belongs to the build — so writing it means waiting for it to appear. A timeout is not an error: the upload stands and only the note is missing |
