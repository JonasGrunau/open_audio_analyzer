...
Dynamics are **`PSR` and `PLR`** — true peak against loudness, over the last
three seconds and over the programme — measured to **Open Dynamic Range**, a
specification this project publishes itself, to the operand, because the AES
names the arithmetic and little else. The application prints the AES names
by default and the specification's own, `ODR-S` and `ODR-I`, from
Settings › Dynamics; nothing but the label changes.
