#!/bin/sh
#
# make_dmg.sh — build Bel for macOS and wrap it in a disk image.
#
# SPDX-License-Identifier: GPL-3.0-or-later
#
# Usage:  sh packaging/macos/make_dmg.sh [--skip-build]
# Output: build/packaging/Bel-<version>-macos-<arch>.dmg
#
# ---------------------------------------------------------------------------
# The Mac App Store is not a target, and that is a decision rather than a gap
#
# Bel's macOS build is deliberately **not sandboxed**. A sandboxed application
# has its HOME redirected into ~/Library/Containers/<bundle id>/Data, which put
# every preset, skin and delivery target inside a container no user goes looking
# in and that BEL_CONFIG_DIR could not escape — see the comment at the top of
...
#                   never for a release.
#   Signed only     BEL_SIGNING_IDENTITY set. Gatekeeper still refuses on a
...
#   Signed+notarised  BEL_SIGNING_IDENTITY and BEL_NOTARY_PROFILE both set.
#                   The only combination a user can double-click.
#
#   BEL_SIGNING_IDENTITY  e.g. "Developer ID Application: Name (TEAMID)"
#   BEL_NOTARY_PROFILE    a `xcrun notarytool store-credentials` profile name
#
...
arch=$(uname -m)
app="build/macos/Build/Products/Release/bel.app"
out="build/packaging"
dmg="$out/Bel-$version-macos-$arch.dmg"
...
if [ -n "${BEL_SIGNING_IDENTITY:-}" ]; then
  echo "==> codesign as $BEL_SIGNING_IDENTITY"
  ...
  find "$app/Contents/Frameworks" -name "*.framework" -maxdepth 1 -print 2>/dev/null |
    while read -r framework; do
      codesign --force --timestamp --options runtime \
        --sign "$BEL_SIGNING_IDENTITY" "$framework"
    done

  codesign --force --timestamp --options runtime \
    --entitlements macos/Runner/Release.entitlements \
    --sign "$BEL_SIGNING_IDENTITY" "$app"

  codesign --verify --strict --verbose=2 "$app"
else
  echo "==> no BEL_SIGNING_IDENTITY: ad-hoc signing"
...
if [ -n "${BEL_NOTARY_PROFILE:-}" ]; then
  echo "==> notarytool submit (this waits for Apple)"
  xcrun notarytool submit "$dmg" --keychain-profile "$BEL_NOTARY_PROFILE" --wait
