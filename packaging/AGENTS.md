...
| `android/play_store.sh` | Upload that bundle to Google Play. Split from the build because `ci.yml` runs it *after* the release is published, exactly as `ios/testflight.sh` is. Talks to the Play Developer API directly — a signed JWT for a service account, five calls, `curl` and `openssl` and `python3` — rather than taking a Ruby toolchain or a third-party action into a release path. |
