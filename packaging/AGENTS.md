...
| `macos/screenshot.sh` | A photograph of the whole desktop application, into `build/packaging/screenshots/desktop.png`, for the website's signal-path section. The macOS counterpart of `ios/screenshots.sh` and driven the same way — the fake DAW plays a real track through the real VST3 into the port the app listens on, so every reading in the picture is one the engine took. It exists because the website's other canvas picture is a headless render of `tools/analyzer-demo`, which has neither bar and no tab strip: a picture of the modules rather than of the program. Needs **Screen Recording** (or `screencapture` returns a black frame and exits 0) and **Accessibility** (the window is placed and sized through System Events, because the app restores whatever geometry it was last left at).  **The application must hold the foreground for the whole wait.** Flutter pauses its ticker when the window is not visible and this application consumes the plugin's stream from that one ticker, so an occluded canvas stops measuring *silently*: the link stays up, the format still reads 44.1 kHz, and the transport sits at 00:00:00:00 indefinitely. Bringing it forward is worse than either, because it then catches up in a burst and the histogram's minute of history arrives in two seconds. The script checks who is frontmost before the shutter and refuses rather than photographing it — so it needs the machine to itself for about ninety seconds. |
