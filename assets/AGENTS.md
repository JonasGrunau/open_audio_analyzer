...
| `brand/play-store-icon.png` | Generated by `packaging/icon/make_icons.dart`. The Google Play listing's icon: 512 px, full bleed, 32 bit. Byte identical to `packaging/android/play_store_icon.png`. |
| `brand/play-store-feature-graphic.png` | Generated by `packaging/android/make_store_graphics.sh`. The Play listing's feature graphic — the *Vorstellungsgrafik* — at 1024 × 500, 24 bit, no alpha. Byte identical to `packaging/android/play_store_feature_graphic.png`. |
...
- **The two `play-store-*` files are the one pair here that a redraw does not
  reach, and only one of them is even downstream of the mark.**
  `play-store-icon.png` is: it is the icon, `make_icons.dart` writes it with the
  rest, and it moves when the drawing does.
  `play-store-feature-graphic.png` is not — it is a card with the product's name
  set on it and carries no tile, no wave and no ramp, so it is downstream of
  `packages/oaa_ui/lib/src/tokens.dart` and `fonts/` instead. Its source is
  `packaging/android/feature_graphic.html` and the thing that renders it is
  `packaging/android/make_store_graphics.sh`; a run of `make_icons.dart` leaves
  it untouched, correctly. Both files here are copies written by those two
  programs, so **editing either one in place is a change the next run silently
  reverts** — the same rule the rest of this directory lives under.
