...
/// Finding Open Audio Analyzer hosts on iOS and iPadOS, through the system's
/// DNS-SD responder.
///
...
/// over a socket Open Audio Analyzer owns, which is what every other platform
/// uses; on an iPad the bind succeeds, the group join succeeds, and then every
/// send to
...
final class OaaBonjour: NSObject, FlutterPlugin, FlutterStreamHandler {
  ...
  private static let serviceType = "_oaa._tcp"

  static func register(with registrar: FlutterPluginRegistrar) {
    let instance = OaaBonjour()
  ...
  /// escaped full name (`Open\032Audio\032Analyzer\032Probe._oaa._tcp.local.`)
  /// — DNS-SD escapes every space, and this name has three — and unescaping
  /// that is more work, and more ways to be wrong, than carrying the name
  /// along.
  final class ResolveContext {
    let owner: OaaBonjour
    let name: String
    init(owner: OaaBonjour, name: String) {
      self.owner = owner
      self.name = name
    }
  }
  ...
  fileprivate func resolved(
    ...
    // published and the system resolver is better placed than Open Audio
    // Analyzer is to decide which of a docked laptop's two interfaces answers
    // to it.
    var hostName = host
  ...
  private func describe(_ error: DNSServiceErrorType) -> String {
    switch error {
    case DNSServiceErrorType(kDNSServiceErr_NoAuth), OaaBonjour.policyDenied:
      return "iPadOS is not letting Open Audio Analyzer search the local network. Allow it "
...
private let browseReply: DNSServiceBrowseReply = {
  ...
  let plugin = Unmanaged<OaaBonjour>.fromOpaque(context).takeUnretainedValue()
...
private let resolveReply: DNSServiceResolveReply = {
  ...
  let resolveContext = Unmanaged<OaaBonjour.ResolveContext>
    .fromOpaque(context)
    .takeUnretainedValue()
  ...
  resolveContext.owner.resolved(
    ...
    // The port arrives in network byte order, which on every device Open Audio
    // Analyzer runs on is the other one.
