...
| `launch_options.dart` | `--config-dir`, `--open-panel`, `--publish` and `--attach`, parsed by hand. Every one of them exists to make something else reviewable; a fifth that drew the in-window FILE button was folded into `fileMenuInWindowProvider`, which a debug build answers true. The last two are the ends of the website's signal-path pair — a desktop publishing and a tablet attached to it — and without them the only way to press PUBLISH and ATTACH from a script was to post mouse events at somebody's machine. |
