...
| `launch_options.dart` | `--config-dir`, `--open-panel`, `--publish`, `--attach` and `--tab`, parsed by hand. Every one of them exists to make something else reviewable; a fifth that drew the in-window FILE button was folded into `fileMenuInWindowProvider`, which a debug build answers true. The last two are the ends of the website's signal-path pair — a desktop publishing and a tablet attached to it — and without them the only way to press PUBLISH and ATTACH from a script was to post mouse events at somebody's machine. |
| `launch_arguments.dart` | The command line, on iOS, over a channel to `ios/Runner/OaaLaunchArguments.swift`. It exists because iOS hands Dart's `main` an empty argument list *and* an empty `Platform.environment`, and answers both without error — so a flag on an iPad silently did nothing and `xcrun simctl launch --args` looked as though it had worked. Every other platform is already told and never calls. |
