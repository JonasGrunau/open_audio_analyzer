/*
 * bel_kweight.h — the ITU-R BS.1770-4 K-weighting filter.
 *
 ...
 * an error here is an error in every loudness number Bel displays.
 *
 ...
 * there is. `bel_kweight_test` asserts that designing at 48 kHz reproduces the
 * printed table, which is what ties our derivation to the standard's.
 */

#ifndef BEL_KWEIGHT_H
#define BEL_KWEIGHT_H
...
static inline double bel_biquad_process(const bel_biquad *c,
                                        bel_biquad_state *s, double x) {
 ...
 * silently zero a real surround channel. Bel has no channel-layout metadata to
 * consult yet; when a device or file source supplies one, this function should
 * take it instead of guessing from the count.
 */
