/*
 * bel_truepeak.c — the 4x polyphase oversampler from BS.1770-4 Annex 2.
 *
 ...
 * down the page.
 */
static const double kPhase[BEL_TRUEPEAK_PHASES][BEL_TRUEPEAK_TAPS] = {
...
void bel_truepeak_init(bel_truepeak *tp, uint32_t channels,
...
static double bel_truepeak_channel(const bel_truepeak *tp, uint32_t channel) {
  const float *delay = tp->delay[channel];
  double largest = 0.0;

  for (uint32_t phase = 0; phase < BEL_TRUEPEAK_PHASES; phase++) {
    const double *taps = kPhase[phase];
    double sum = 0.0;

    for (uint32_t tap = 0; tap < BEL_TRUEPEAK_TAPS; tap++) {
      /* Walk the delay line backwards from the newest sample, wrapping. */
      const uint32_t index =
          (tp->pos + BEL_TRUEPEAK_TAPS - tap) % BEL_TRUEPEAK_TAPS;
...
void bel_truepeak_process(bel_truepeak *tp, const float *interleaved,
  ...
  for (uint32_t frame = 0; frame < frames; frame++) {
    tp->pos = (tp->pos + 1) % BEL_TRUEPEAK_TAPS;

    for (uint32_t channel = 0; channel < channels; channel++) {
      tp->delay[channel][tp->pos] = interleaved[(size_t)frame * channels + channel];

      const double peak = bel_truepeak_channel(tp, channel);
    ...
    if (++tp->frames_in_subblock >= tp->frames_per_subblock) {
      ...
      tp->history_write = (tp->history_write + 1) % BEL_TRUEPEAK_HISTORY;
      if (tp->history_filled < BEL_TRUEPEAK_HISTORY) {
        tp->history_filled++;
      }
      tp->frames_in_subblock = 0;
      tp->subblock_peak = 0.0;
...
double bel_truepeak_max(const bel_truepeak *tp) { return tp->peak_max; }
