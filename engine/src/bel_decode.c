/*
 * bel_decode.c — a file on disk, as blocks of float samples.
 *
 ...
 * `bel_engine_push`, which runs the same DSP graph a capture device drives.
 *
 * ---------------------------------------------------------------------------
 * Why the decoder is in C rather than Dart
 *
 * Three consumers analyse files — the app's drag-and-drop panel, the `bel`
 ...
 * `bel_analyse` through the same buffers, which is exactly what decoding into
 * BEL_SOURCE_PUSH does. A Dart-side decoder would be a second path, and a
 ...
 *
 * 2. **This is a separate translation unit from bel_device.c on purpose.**
 ...
 * engine. Everything else that needs them just calls through bel_file_*. */
 ...
 */
static wchar_t *bel_widen(const char *utf8) {
...
static uint32_t bel_format_from_container(drwav_container container) {
  switch (container) {
    case drwav_container_aiff:
      return BEL_FILE_FORMAT_AIFF;
    case drwav_container_w64:
      return BEL_FILE_FORMAT_W64;
    case drwav_container_rf64:
      return BEL_FILE_FORMAT_RF64;
    case drwav_container_riff:
    case drwav_container_rifx:
    default:
      return BEL_FILE_FORMAT_WAV;
  }
}

static int bel_try_wav(bel_file *file, const char *path) {
#if defined(_WIN32)
  wchar_t *wide = bel_widen(path);
  if (wide == NULL) {
    return 0;
  }
  ...
  file->info.format = bel_format_from_container(file->wav.container);
  return 1;
}

static int bel_try_flac(bel_file *file, const char *path) {
#if defined(_WIN32)
  wchar_t *wide = bel_widen(path);
  if (wide == NULL) {
    return 0;
  ...
  file->info.format = BEL_FILE_FORMAT_FLAC;
  return 1;
}

static int bel_try_mp3(bel_file *file, const char *path) {
#if defined(_WIN32)
  wchar_t *wide = bel_widen(path);
  if (wide == NULL) {
    return 0;
  ...
  file->info.format = BEL_FILE_FORMAT_MP3;
  return 1;
}

bel_file *bel_file_open(const char *path) {
  if (path == NULL || path[0] == '\0') {
    return NULL;
  }

  bel_file *file = (bel_file *)calloc(1, sizeof(bel_file));
  if (file == NULL) {
    return NULL;
  }

  /* Order matters — see the note at the top of this file. */
  if (!bel_try_wav(file, path) && !bel_try_flac(file, path) &&
      !bel_try_mp3(file, path)) {
    free(file);
    return NULL;
  }
...
int32_t bel_file_get_info(const bel_file *file, bel_file_info *out) {
...
int32_t bel_file_seek(bel_file *file, uint64_t frame) {
  if (file == NULL) {
    return BEL_ERR_INVALID_ARGUMENT;
  }
  ...
  return ok ? BEL_OK : BEL_ERR_UNSUPPORTED;
}

void bel_file_close(bel_file *file) {
  if (file == NULL) {
    return;
  }
