/*
 * bel_kweight.c — designing the BS.1770-4 K-weighting filter.
 *
 ...
 * from this prototype (which `bel_kweight_test` checks) needs the prototype at
 * comparable precision. Rounding f0 to 1682 Hz moves the printed coefficients
 * in their sixth decimal place.
 */
#define BEL_SHELF_F0 1681.974450955533
#define BEL_SHELF_GAIN_DB 3.999843853973347
#define BEL_SHELF_Q 0.7071752369554196
...
#define BEL_SHELF_VB_EXPONENT 0.4996667741545416

#define BEL_HIGHPASS_F0 38.13547087602444
#define BEL_HIGHPASS_Q 0.5003270373238773

void bel_kweight_design(double sample_rate, bel_biquad *shelf,
                        bel_biquad *highpass) {
  /* --- Stage 1: high-frequency shelf ---------------------------------- */
  {
    const double k = tan(BEL_PI * BEL_SHELF_F0 / sample_rate);
    const double vh = pow(10.0, BEL_SHELF_GAIN_DB / 20.0);
    const double vb = pow(vh, BEL_SHELF_VB_EXPONENT);
    const double a0 = 1.0 + k / BEL_SHELF_Q + k * k;

    shelf->b0 = (vh + vb * k / BEL_SHELF_Q + k * k) / a0;
    shelf->b1 = 2.0 * (k * k - vh) / a0;
    shelf->b2 = (vh - vb * k / BEL_SHELF_Q + k * k) / a0;
    shelf->a1 = 2.0 * (k * k - 1.0) / a0;
    shelf->a2 = (1.0 - k / BEL_SHELF_Q + k * k) / a0;
  }

  /* --- Stage 2: RLB high-pass ------------------------------------------ */
  {
    const double k = tan(BEL_PI * BEL_HIGHPASS_F0 / sample_rate);
    const double denominator = 1.0 + k / BEL_HIGHPASS_Q + k * k;
    ...
    highpass->a2 = (1.0 - k / BEL_HIGHPASS_Q + k * k) / denominator;
  }
}

double bel_channel_weight(uint32_t index, uint32_t count) {
