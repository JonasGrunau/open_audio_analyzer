...
void oaa_ring_clear(oaa_ring *ring) {
  ...
  oaa_atomic_store_release(&ring->read_index, 0);
  oaa_atomic_store_release(&ring->write_index, 0);
  oaa_atomic_store_release(&ring->dropped, 0);
}

uint32_t oaa_ring_available(const oaa_ring *ring) {
  const uint32_t write = oaa_atomic_load_acquire(&ring->write_index);
  const uint32_t read = oaa_atomic_load_acquire(&ring->read_index);
...
uint32_t oaa_ring_dropped(const oaa_ring *ring) {
  return oaa_atomic_load_acquire(&ring->dropped);
}
...
uint32_t oaa_ring_write(oaa_ring *ring, const float *interleaved,
  ...
  if (writable < frames) {
    ...
    oaa_atomic_add(&ring->dropped, frames - writable);
  }
  ...
  oaa_atomic_store_release(&ring->write_index, write + writable);
  return writable;
}
...
uint32_t oaa_ring_read(oaa_ring *ring, float *interleaved, uint32_t frames) {
  ...
  oaa_atomic_store_release(&ring->read_index, read + readable);
  return readable;
}
