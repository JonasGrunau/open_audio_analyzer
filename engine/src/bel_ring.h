/*
 * bel_ring.h — the single-producer, single-consumer ring between the audio
...
typedef struct {
  ...
  bel_atomic_u32 write_index;
  bel_atomic_u32 read_index;
 ...
 * `bel_ring_clear` is not — it moves both indices and would race the callback
