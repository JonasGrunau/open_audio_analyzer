/*
 * bel_loudness.c — EBU R 128 gating, integration and loudness range.
 *
...
#define BEL_LOUDNESS_OFFSET (-0.691)

/* Absolute gate, LUFS. */
#define BEL_GATE_ABSOLUTE (-70.0)

/* Relative gate below the ungated mean, in LU. Integrated loudness uses 10;
 * loudness range uses 20. */
#define BEL_GATE_RELATIVE_INTEGRATED 10.0
#define BEL_GATE_RELATIVE_RANGE 20.0

void bel_loudness_init(bel_loudness *l, uint32_t channels,
                       uint32_t sample_rate) {
  memset(l, 0, sizeof(*l));

  l->channels = channels;
  l->frames_per_subblock = sample_rate / BEL_SUBBLOCK_DIVISOR;
  if (l->frames_per_subblock == 0) {
    l->frames_per_subblock = 1;
  }

  bel_kweight_design((double)sample_rate, &l->shelf, &l->highpass);

  for (uint32_t c = 0; c < channels; c++) {
    l->weight[c] = bel_channel_weight(c, channels);
  }
}
...
static double loudness_from_energy(double energy) {
  ...
  return BEL_LOUDNESS_OFFSET + 10.0 * log10(energy);
}

static int bin_of_loudness(double loudness) {
  const int bin = (int)floor((loudness - BEL_HIST_MIN_LUFS) / BEL_HIST_STEP);
  if (bin < 0) {
    return -1; /* below the absolute gate; caller drops it */
  }
  return bin >= BEL_HIST_BINS ? BEL_HIST_BINS - 1 : bin;
}
...
static double loudness_of_bin(int bin) {
  return BEL_HIST_MIN_LUFS + ((double)bin + 0.5) * BEL_HIST_STEP;
}
 ...
 */
static double window_energy(const bel_loudness *l, uint32_t subblocks) {
...
static void file_block(bel_hist_bin *histogram, double energy) {
  const double loudness = loudness_from_energy(energy);
  if (!(loudness > BEL_GATE_ABSOLUTE)) {
...
static void close_subblock(bel_loudness *l) {
  ...
  const double gating_energy = window_energy(l, BEL_MOMENTARY_SUBBLOCKS);
  if (gating_energy >= 0.0) {
    file_block(l->gating, gating_energy);
  }

  const double shortterm_energy = window_energy(l, BEL_SHORTTERM_SUBBLOCKS);
  ...
  for (uint32_t frame = 0; frame < frames; frame++) {
    for (uint32_t c = 0; c < channels; c++) {
      ...
      const double shelved =
          bel_biquad_process(&l->shelf, &l->shelf_state[c], input);
      const double weighted =
          bel_biquad_process(&l->highpass, &l->highpass_state[c], shelved);
...
double bel_loudness_momentary(const bel_loudness *l) {
  const double energy = window_energy(l, BEL_MOMENTARY_SUBBLOCKS);
  if (energy < 0.0) {
    return NAN;
  }
  const double loudness = loudness_from_energy(energy);
  return loudness == -HUGE_VAL ? (double)BEL_DB_FLOOR : loudness;
}

double bel_loudness_shortterm(const bel_loudness *l) {
  const double energy = window_energy(l, BEL_SHORTTERM_SUBBLOCKS);
  if (energy < 0.0) {
    return NAN;
  }
  const double loudness = loudness_from_energy(energy);
...
static double gated_mean(const bel_hist_bin *histogram, int from_bin,
                         uint64_t *count_out) {
  double energy = 0.0;
  uint64_t count = 0;

  for (int bin = from_bin < 0 ? 0 : from_bin; bin < BEL_HIST_BINS; bin++) {
...
double bel_loudness_integrated(const bel_loudness *l) {
  ...
  const double relative =
      loudness_from_energy(ungated_mean) - BEL_GATE_RELATIVE_INTEGRATED;
...
static int range_gate_start(const bel_loudness *l, uint64_t *total_out,
  ...
  const double relative =
      loudness_from_energy(ungated_mean) - BEL_GATE_RELATIVE_RANGE;
