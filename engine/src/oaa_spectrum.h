 ...
 * territory in between. oaa_spectrum_map_bands() has the argument in full.
 *
 * ---------------------------------------------------------------------------
...
#ifndef OAA_SPECTRUM_H
#define OAA_SPECTRUM_H
 ...
 * OAA_FFT_WINDOW is the resolution; it is also the ring size, so it must stay
 * a power of two. */
#define OAA_FFT_WINDOW 4096
#define OAA_FFT_HOP 1024

/* Points per transform: the window, zero-padded. Only the sampling of the
 * result gets finer — see the header comment. */
#define OAA_FFT_TRANSFORM 16384
...
#define OAA_SPECTRUM_HOLD_SECONDS 1.5f
#define OAA_SPECTRUM_FALL_DB_PER_SECOND 12.0f
...
typedef struct {
  ...
  float *window;
  float *input; /* channels * OAA_FFT_WINDOW, channel-major */
  uint32_t write;
  uint32_t filled;    /* frames seen, saturating at OAA_FFT_SIZE */
   ...
   *
   * `frame` is OAA_FFT_TRANSFORM long and only its first OAA_FFT_WINDOW
  ...
  uint16_t band_first[OAA_SPECTRUM_BANDS];
  uint16_t band_last[OAA_SPECTRUM_BANDS];
  ...
  float band_lerp[OAA_SPECTRUM_BANDS];

  float band_db[OAA_SPECTRUM_BANDS];
 ...
 * pffft refuses OAA_FFT_TRANSFORM — neither is recoverable at the call site. */
int oaa_spectrum_init(oaa_spectrum *s, uint32_t channels, uint32_t sample_rate);
