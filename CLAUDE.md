...
| `packages/oaa_ui/lib/src/point_buckets.dart` | Marks sorted by the colour they are drawn in, one call per colour. What lets the stereo cloud redraw its whole ring of hits every published frame instead of accumulating it into an image. The spectrogram drew through it too until its run counts on real material outgrew it — see its header. |
...
| `website/` | `open-audio-analyzer.com` — a static Astro site that is also where the documentation is published, rendered from this repository's own Markdown in place. Two Flutter web targets give it its pictures — one photographs a module at a time, the other is a live canvas of eight — and both replay one recording rather than a mock: a Dart CLI measures a real track through the engine, and the `ReplaySource` they share plays it back, so a still and the live demo cannot disagree about what the material did. Built by the `website` job in `ci.yml` on every event and deployed by `website-deploy`, which is a separate job so that it can wait for the release the site describes: a push to `main` deploys only when `pubspec.yaml` already names a released version, and a tag deploys once `publish` has published one. It was deployed by hand until 0.11.0, and deployed a front page naming an unreleased version until 0.15.0. | GPL-3.0-or-later; fonts SIL OFL 1.1 |
...
| The definition of `ODR-S` or `ODR-I` — an operand, a gate, a window, the statistic | `docs/ODR.md` **with its version bumped and a row in § 9**, the matching case in `conformance_test.dart`, then the three above. The specification is what a third party implements; a definition that moved without the version moving is two standards under one number |
| A band in ODR Annex A.2 — an edge, a name, an anchor | **Three copies**: the annex, the table in `README.md`'s ODR section, and `BANDS` / `ANCHORS` in `website/src/pages/index.astro`, which draws them to scale. All informative, none regenerated, and the band *names* are also what `exportReportText` prints after `ODR-I` — a fourth place, in `packages/oaa_core/lib/src/report_export.dart`, with its own test |
