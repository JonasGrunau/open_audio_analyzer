...
| `docs/ODR.md` | **Open Dynamic Range**, the project's own dynamics standard — `ODR-S` and `ODR-I` defined to the operand, with numbered conformance cases. Normative and versioned in its own header: the definition changes only with a version bump and a revision-history row, and `packages/oaa_engine/test/conformance_test.dart` asserts every case in its § 7. `METRICS.md` and the README summarise it and point at it; they do not restate it, so there is one text to drift from. Its prose is CC BY 4.0, the one file in `docs/` that is not GPL, because a specification nobody may reproduce is not open. |
