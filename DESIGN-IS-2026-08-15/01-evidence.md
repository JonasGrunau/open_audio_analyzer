...
(`packages/bel_core/lib/src/layout.dart:10`) and dispatched through a single
...
**Top toolbar** carries 10 items (`lib/src/app/bel_app.dart:430-478`): source
...
- Refresh rate — toolbar `_FpsPicker` (`bel_app.dart:447`) and Settings
  "Refresh rate" (`lib/src/panels/settings_panel.dart:170-177`).
- Delivery target — toolbar `_CalibrationPicker` (`bel_app.dart:444`) and
...
(`lib/src/canvas/grid_canvas.dart:270-310`, `packages/bel_ui/lib/src/module_frame.dart:69-105`).
...
(`packages/bel_ui/lib/src/tokens.dart:16-48`):
`[2, 4, 8, 12, 16, 24, 32, 48, 64]`.

**Enforcement is total.** Repo-wide search for raw numeric literals in
`EdgeInsets.all|symmetric|only|fromLTRB` across `lib/` and `packages/bel_ui/`
...
`bel_core` (`packages/bel_core/lib/src/skin.dart:25-38`) and `bel_ui`
...
**Decoration inventory** across `lib/` and `packages/bel_ui/`:
...
One border weight (`BelStroke.hairline = 1`), with `mark = 1.5` reserved for
...
statement (`packages/bel_ui/lib/src/readout.dart:121`,
...
| disabled | **yes** | `packages/bel_ui/lib/src/panel.dart:469, 504-508` — colour, cursor and null callback; doc at `:453-454` |
| **focus** | **NO** | see §5 |
...
(`packages/bel_ui/lib/src/persistence_layer.dart`); the `?` toolbar button
exists solely because Bel draws its own chrome and so has no menu bar to read
shortcuts from (`bel_app.dart:459-470`, comment); notice timer cancelled on
dispose because a 4-second fuse outlives a closing tab
...
`best|fastest|powerful|seamless|effortless|beautiful|world-class|revolutionary|instantly|just works`
across `README.md` returns **one** hit: `README.md:12` — *"the modular canvas is
the best interaction model anybody has found for this problem."* Prose in the
design document, not a product claim, and it is an argument about a borrowed
pattern rather than about Bel.
...
(`packages/bel_core/lib/src/metric.dart:11-27`). All domain-correct for the
...
`stereo_cloud.dart:123`, `report_panel.dart:371`, `bel_app.dart:727`).

**Honesty machinery, verified present:**
- NaN-never-zero, with the reasoning written down:
  `packages/bel_core/lib/src/metric.dart:60-70` — *"`0.0` — the other obvious
  placeholder — looks like a measurement."*
- Engine-side flags: `BEL_FLAG_LOUDNESS_UNAVAILABLE`,
  `BEL_FLAG_SPECTRUM_UNAVAILABLE` (`engine/include/bel/bel.h:169, 175`).
- Em dash rendering: `metric.dart:66`, `report.dart:375`, `readout.dart:24, 121`,
  `lib/src/data/metric_reader.dart:53-57`.
- **Refusal to fabricate a competitor's metric** — `README.md:117-125`: Decibel's
  *TrueDyn* is proprietary and undocumented, *"so any claim to match it would be
  a guess presented as a measurement. Bel does not implement it."* Bel publishes
  `DR-S`/`DR-I` from stated definitions instead.
...
- **Paragraph re-layout avoided** unless the formatted string, colour or size
  changed (`packages/bel_ui/lib/src/readout.dart:69-85`).
...
- **Animation count on an idle screen: 0 decorative.** Zero `AnimationController`,
  `Tween`, `CurvedAnimation` or `AnimatedContainer` in `lib/` or `bel_ui/`. The
...
- **`prefers-reduced-motion` / `disableAnimations` / `highContrast` /
  `textScaler`: 0 hits** across `lib/` and `packages/bel_ui/`.

---

## §5 Accessibility

- **`Semantics(` widgets in the entire app: 0.** (`lib/` + `packages/bel_ui/`.)
- **Focus state: absent from every custom control.** All five interactive
  primitives in `packages/bel_ui/lib/src/panel.dart` are
  `MouseRegion` + `GestureDetector` with no `FocusableActionDetector`, no
  `Focus`, no `FocusNode`, and no focus ring:
  `PanelListRow:319-325`, `SegmentedControl:413`, `BelButton:483-487`,
  `BelToggle:532-534`, `_IconTarget:652-654`.
  The sole focus participant is `BelTextField` (`:580, 590, 614` — `autofocus`).
...
- **Keyboard reachability of primary actions:** app-level shortcuts exist and are
  ...
  (`bel_app.dart:459-470`). **But** no button, toggle, segmented control or list
...
- **Hit-target discipline** is handled and documented — painted chrome
  ...
  (`packages/bel_ui/lib/src/module_frame.dart:31`, and `CLAUDE.md`).
...
1. **Deterministic** — `flutter test` renders the real canvas over a live
   `BelEngine` at a fixed 1600×940 @2×, with the bundled Inter and JetBrains
...
2. **Live** — the built `bel.app` on macOS, driven by `--open-panel=settings`
...
any grid size over a live `BelEngine`, `_pumpSettings` opens the settings panel
through `showBelPanel` with the boundary **above** `MaterialApp` (below it the
...
| 5 — fast paths | fps chip and delivery target both duplicated the panel | fps chip removed; delivery target kept, with the rule written into `bel_app.dart` |
