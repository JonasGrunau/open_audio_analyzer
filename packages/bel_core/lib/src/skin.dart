...
///
/// This lives in `bel_core` and therefore stores integers rather than
/// `dart:ui` `Color`s: the tablet display, the CLI and the plugin all parse
/// presets and none of them may drag Flutter in. `bel_ui` owns the one adapter
/// that turns these into a `BelColors`.
library;

/// The roles a skin assigns a colour to.
///
/// Every value here is a field of `BelColors` in `bel_ui`, and the pairing is
/// asserted by a test in that package rather than by a comment here — the two
/// lists living in different packages is the price of `bel_core` staying free of
...
abstract final class BuiltInSkins {
  ...
  ///
  /// These are the same values as `BelColors.precisionInstrument`, and a test in
  /// `bel_ui` asserts they have not drifted apart.
  static const Skin precisionInstrument = Skin(
    ...
    colors: {
      ...
      SkinColor.hairlineStrong: 0xFF2E343C,
      ...
      SkinColor.meterTrack: 0xFF1A1E23,
  ...
  static const Skin daylight = Skin(
    ...
    colors: {
      ...
      SkinColor.hairlineStrong: 0xFFB2BAC3,
      ...
      SkinColor.meterTrack: 0xFFE1E5EA,
