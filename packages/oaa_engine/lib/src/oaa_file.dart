...
/// `OaaEngine.push`, which is the same `oaa_analyse` a capture device drives,
...
enum OaaFileFormat {
  ...
  const OaaFileFormat(this.value, this.label);
  ...
  /// [OaaFileInfo.bitsPerSample] has no answer for it.
  bool get isLossy => this == OaaFileFormat.mp3;

  static OaaFileFormat fromValue(int value) {
    for (final format in OaaFileFormat.values) {
      if (format.value == value) return format;
    }
    return OaaFileFormat.unknown;
  }
}
...
class OaaFileInfo {
  ...
  @override
  String toString() => 'OaaFileInfo(${describe()})';
}

/// Thrown when a file cannot be opened or decoded.
class OaaFileException implements Exception {
  const OaaFileException(this.message, [this.path]);
  ...
  String toString() => path == null
      ? 'OaaFileException: $message'
      : 'OaaFileException: $message ($path)';
}
...
class OaaFile {
  OaaFile._(this._handle, this.info, this._blockFrames)
  ...
  ///
  /// Throws [OaaFileException] if the file cannot be read or is not a format
  /// this build decodes.
  static OaaFile open(String path, {int blockFrames = 1024}) {
    if (blockFrames <= 0) {
      throw ArgumentError.value(blockFrames, 'blockFrames', 'must be positive');
    }

    final nativePath = path.toNativeUtf8();
    final Pointer<native.oaa_file> handle;
    try {
      handle = native.oaa_file_open(nativePath.cast<Char>());
    } finally {
    ...
    final infoStruct = calloc<native.oaa_file_info>();
    try {
      final status = native.oaa_file_get_info(handle, infoStruct);
      if (status != 0) {
        native.oaa_file_close(handle);
        throw OaaFileException('could not read file information', path);
      }

      final ref = infoStruct.ref;
      final info = OaaFileInfo(
        sampleRate: ref.sample_rate,
        channels: ref.channels,
        frames: ref.frames,
        durationSeconds: ref.duration_seconds,
        format: OaaFileFormat.fromValue(ref.format),
        bitsPerSample: ref.bits_per_sample,
      );
      return OaaFile._(handle, info, blockFrames);
    } finally {
      calloc.free(infoStruct);
    }
  }

  final Pointer<native.oaa_file> _handle;
  ...
  /// Frames decoded so far. Divide by [OaaFileInfo.frames] for progress.
  ...
  /// thousands of buffers. Pass it straight to `OaaEngine.push`, which copies
  /// what it needs; hold on to it and you are reading whatever the next block
  /// decoded into.
  Float32List? readBlock() {
    _checkOpen();

    final framesRead = native.oaa_file_read(_handle, _buffer, _blockFrames);
  ...
  bool seek(int frame) {
    _checkOpen();
    final ok = native.oaa_file_seek(_handle, frame) == 0;
  ...
  void close() {
    if (_closed) return;
    _closed = true;
    native.oaa_file_close(_handle);
    calloc.free(_buffer);
  }

  void _checkOpen() {
    if (_closed) {
      throw StateError('OaaFile has been closed');
    }
  }

  @override
  String toString() => 'OaaFile(${info.describe()})';
}
