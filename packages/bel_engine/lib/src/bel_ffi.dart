...
import '../bel_engine_bindings_generated.dart' show bel_engine;
...
/// `package:bel_engine/src/bel_ffi.dart` — not what the build hook emitted.
...
const _assetId = 'package:bel_engine/bel_engine_bindings_generated.dart';
...
/// wasted here — `bel_snapshot_acquire` does an atomic load, a memcpy and a
...
/// the runtime. `bel_snapshot_acquire` satisfies both by construction: it is a
...
/// with no diagnostic, so the invariant is restated in bel.h next to the
/// function itself.
@Native<Uint64 Function(Pointer<bel_engine>)>(
  symbol: 'bel_snapshot_acquire',
  assetId: _assetId,
  isLeaf: true,
)
external int belSnapshotAcquireLeaf(Pointer<bel_engine> engine);
