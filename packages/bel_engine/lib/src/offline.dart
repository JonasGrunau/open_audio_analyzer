...
/// in `bel_core`, which must never import `dart:ffi`. Keeping the split here
...
/// [BelCancelToken.fromAddress], and the worker sees a [cancel] the instant it
...
class OfflineResult {
  ...
  /// `DR-I` as Bel defines it in docs/METRICS.md — true peak max minus
  /// integrated loudness. Not Decibel's proprietary "TrueDyn", and not claimed
  /// to match it.
...
///
/// Runs the same `bel_analyse` a capture device drives, at whatever speed the
...
OfflineResult analyseFile(
  ...
  final file = BelFile.open(path, blockFrames: blockFrames);

  if (file.info.channels > kBelMaxChannels) {
    final channels = file.info.channels;
    file.close();
    throw BelFileException(
      'the engine carries at most $kBelMaxChannels channels (7.1) and this '
      'file has $channels',
      path,
    );
  }
