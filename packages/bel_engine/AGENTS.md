...
| `lib/bel_engine.dart` | The `BelEngine` facade and the zero-copy typed views. Implements `bel_core`'s `MeterSource`. |
| `lib/src/bel_file.dart` | `bel_file_*` as Dart: open a file, read blocks, close. Decoding only — it measures nothing. |
