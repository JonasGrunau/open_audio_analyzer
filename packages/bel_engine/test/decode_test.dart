...
// same `bel_analyse` rather than a second implementation of it. If this test
...
Map<String, double> _readings(BelEngine engine) => {
...
Map<String, double> _analyseFile(File file) {
  final decoded = BelFile.open(file.path, blockFrames: _blockFrames);
  final engine = BelEngine.start(
    source: BelSource.push,
...
void main() {
  ...
  setUp(() => temp = Directory.systemTemp.createTempSync('bel_decode_test'));
  tearDown(() => temp.deleteSync(recursive: true));

  group('opening', () {
    test('reads format, rate, channels and duration from a float WAV', () {
      final file = _writeFloatWav(temp, 'tone.wav', _sine(2.0), 2);
      final decoded = BelFile.open(file.path);

      expect(decoded.info.format, BelFileFormat.wav);
    ...
    test('a file that is not audio throws rather than returning empty', () {
      ...
      expect(() => BelFile.open(junk.path), throwsA(isA<BelFileException>()));
    });

    test('a missing file throws', () {
      expect(
        () => BelFile.open('${temp.path}/does_not_exist.wav'),
        throwsA(isA<BelFileException>()),
      );
    });
  });
  ...
  group('reading', () {
    ...
    test('reading after close is an error rather than a native crash', () {
      ...
      final decoded = BelFile.open(file.path)..close();
    ...
    test('a stereo file reads identically pushed and decoded', () {
      ...
      for (final key in inMemory.keys) {
        expect(
          ...
          inMemory[key],
          reason:
              '$key differs: decoding a file must reach the same bel_analyse '
