...
int _loudestBand(BelEngine engine) {
  var loudest = 0;
...
void main() {
  ...
  test('silence reads as the floor, not as zero', () {
    ...
    expect(engine.hasSpectrum, isTrue);
    for (var band = 0; band < kBelSpectrumBands; band++) {
      expect(engine.spectrum[band], kBelDbFloor);
    }
  });
  ...
  test('bands above Nyquist stay empty rather than wrapping', () {
    ...
    for (var band = nyquistBand; band < kBelSpectrumBands; band++) {
      expect(engine.spectrum[band], kBelDbFloor, reason: 'band $band');
    }
  });

  test('the frequency mapping round-trips', () {
    for (var band = 0; band < kBelSpectrumBands; band += 37) {
