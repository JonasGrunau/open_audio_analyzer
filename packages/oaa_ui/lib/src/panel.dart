...
/// `OaaTheme` was under `home` and re-provided here as a *copy taken when the
...
/// one place that mattered. `OaaApp` installs the palette through
...
/// worth.
Future<T?> showOaaPanel<T>({
  required BuildContext context,
  required WidgetBuilder builder,
}) {
  final captured = OaaTheme.of(context);
  ...
  return showGeneralDialog<T>(
    ...
    pageBuilder: (context, _, _) {
      final colors = OaaTheme.maybeOf(context) ?? captured;
...
class _Hairline extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ColoredBox(
    color: colors.hairline,
    child: const SizedBox(height: OaaStroke.hairline, width: double.infinity),
  );
}
...
/// A section heading is [OaaType.label] at 10 px and its note is
/// [OaaType.caption] at 11 px, so the heading is the *smallest* text in its own
    ...
    return Column(
      ...
      children: [
        ...
        if (note != null) ...[
          ...
          Text(note!, style: OaaType.caption.copyWith(color: colors.textFaint)),
        ],
...
///
/// The same caption-on-[OaaColors.textFaint] that a row's note is, written once
...
class PanelNote extends StatelessWidget {
  ...
  /// A [OaaMark] in the margin, in the note's own tone.
  ///
  ...
  Widget build(BuildContext context) {
    ...
    final label = Text(text, style: OaaType.caption.copyWith(color: color));
...
class PanelListRow extends StatelessWidget {
  ...
  /// A [OaaMark] down the left of the row, in the title's own colour.
  ///
  /// A `OaaMark` rather than a widget slot: a row that could be handed any
            ...
            for (final segment in segments)
              OaaFocusable(
                ...
                builder: (context, hovered, focused) => Container(
                  ...
                  // Uppercase in [OaaType.label], like [OaaButton] and the
...
///
/// **Not a `PopupMenuButton`.** That builds its own `InkWell`, and the Open Audio Analyzer
...
/// [OaaFocusable] puts all three back and costs about fifteen lines.
///
/// The value is [OaaColors.textPrimary]: a menu whose current setting is
  ...
  Future<void> _open(BuildContext context) async {
    ...
    final chosen = await showMenu<T>(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: OaaRadius.allSm,
        side: BorderSide(color: colors.hairline, width: OaaStroke.hairline),
      ),
      items: [
  ...
  Widget build(BuildContext context) {
    ...
    return OaaFocusable(
      ...
      builder: (context, hovered, focused) => Container(
        ...
        padding: const EdgeInsets.symmetric(horizontal: OaaControl.padding),
        decoration: BoxDecoration(
...
class OaaButton extends StatelessWidget {
  const OaaButton({
    required this.label,
  ...
  /// than no button — and, since this became a [OaaFocusable], a disabled
...
class OaaToggle extends StatelessWidget {
  const OaaToggle({
    required this.value,
    required this.onChanged,
    this.semanticLabel,
...
class OaaTextField extends StatelessWidget {
  const OaaTextField({
  ...
  Widget build(BuildContext context) {
    ...
    final style = numeric
        ...
        : OaaType.body.copyWith(color: colors.textPrimary);
