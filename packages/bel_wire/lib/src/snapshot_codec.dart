...
///
/// The layout was derived mechanically from `bel_snapshot` in
/// `engine/include/bel/bel.h` at ABI 3 — declaration order, top to bottom,
...
/// protocol version 1. `bel_snapshot` will grow fields and bump
/// `BEL_ABI_VERSION`, which is a private matter between the engine and the
...
abstract final class SnapshotWire {
  ...
  // The `BEL_FLAG_*` bits, which is the one place the wire reproduces a
  ...
  static void encode(MeterSource source, ByteData into, {int offset = 0}) {
    ...
    _writeFloats(into, offset + offsetSpectrum, source.spectrum);
    _writeFloats(into, offset + offsetSpectrumPeak, source.spectrumPeak);
    _writeFloats(into, offset + offsetSpectrumPan, source.spectrumPan);
    _writeFloats(into, offset + offsetScope, source.scope);
    _writeFloats(into, offset + offsetHistogram, source.histogram);
  }
...
class SnapshotFrame {
  SnapshotFrame() {
    WireFrame.writeHeader(
      _view,
      WireFrameType.snapshot,
      SnapshotWire.payloadBytes,
    );
  }

  /// The complete frame — header and payload — ready to write to a socket.
  final Uint8List bytes = Uint8List(
    WireFrame.headerBytes + SnapshotWire.payloadBytes,
  );

  late final ByteData _view = ByteData.view(bytes.buffer);

  /// Overwrites the payload with [source]'s current reading.
  void encode(MeterSource source) =>
      SnapshotWire.encode(source, _view, offset: WireFrame.headerBytes);
}
