...
/// `plugin/test/golden/wire_v1.bin` — a file written by `bel_wire_fixture`,
/// compiled from `plugin/src/BelWire.cpp` — and asserts the values that went
/// in come back out.
///
...
void main() {
  group('the C++ plugin golden', () {
    ...
    test('carries three frames of the expected sizes', () {
      // A 32-byte hello block plus a 22-byte UTF-8 name, an 88-byte transport
      // and a 15,056-byte snapshot, each behind a 12-byte envelope.
      final bytes = _goldenBytes().length;
      expect(bytes, (12 + 32 + 22) + (12 + 88) + (12 + 15056));
      expect(bytes, 15234);
    });
    ...
    test('names itself, in UTF-8, without mangling non-ASCII', () {
      ...
      expect(hello!.producerName, 'Bel plugin — fixture');
    });
    ...
    test('preserves negative infinity rather than clamping to the floor', () {
      // Digital silence. Clamping it to BEL_DB_FLOOR would be a different
...
Uint8List _goldenBytes() {
  ...
  for (var depth = 0; depth < 5; depth++) {
    final candidate = File('${directory.path}/plugin/test/golden/wire_v1.bin');
  ...
  throw StateError(
    'plugin/test/golden/wire_v1.bin not found from ${Directory.current.path}. '
    'Regenerate it with the bel_wire_fixture target in plugin/.',
  );
}
