...
| `src/layout.dart` | `GridRect`, `ModuleKind`, `ModuleSpec`, `TabSpec`, `PresetSpec`, and the typed readings of `ModuleSpec.options` — `SpectrumResponse`, `SpectrumTilt`, `HistogramSmoothing`, `DistributionScale`, `ScopeTimeBase`, `ScopeSync`, `ScopeDivision`, `ScopeGrid`, `ScopeStereo`, `ScopeTrigger`, `ScopeThreshold`, `ScopeZoom`. |
