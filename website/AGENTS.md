...
| `scripts/render-flow.mjs` | `npm run flow`: the three photographs behind the signal-path section, from three different places. The plugin panel comes out of `plugin/build/oaa_editor_snapshot`, which needs no DAW and no screen-recording grant; neither of the other two is shot here at all. The desktop plate is the **whole application window**, written by `packaging/macos/screenshot.sh`, and it is deliberately not the headless `tools/analyzer-demo` render the hero uses — that one has no chrome, so it photographs the modules and not the program. The tablet plate is `packaging/ios/screenshots.sh`'s `01-loudness.png` — the clean canvas, not `05-remote-display.png`, which is the frame with a panel over it and which currently photographs the canvas anyway because that script's tap coordinates have drifted. Both need a machine, a permission and a window server; this only picks up what they left in `build/packaging/screenshots/`. `--only=` does one stage. Each missing stage prints what would make it. |
