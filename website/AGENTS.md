...
| `src/data/window-shots.json` | The intrinsic size of each window shot and the rungs it was published at, written beside the pictures by `npm run window`, for the same reason as the still's and the plates': an `<img>` needs a width and a height before anything is fetched, and a number typed into the markup is a number that goes wrong the first time a source is reshot. **No page reads it yet** — the pictures were taken before there was a section to put them in, and this is what a section will size and build its `srcset` from when there is one. |
...
| `scripts/render-flow.mjs` | `npm run flow`: the three photographs behind the signal-path section, from two different places, and nothing is shot here. The plugin panel comes out of `plugin/build/oaa_editor_snapshot`, which needs no DAW and no screen-recording grant. The desktop and tablet plates are both written by `packaging/signal_path.sh`, **in one session**: the tablet is a remote display attached to that desktop, so the two are one measurement photographed twice rather than two that happen to agree. The desktop plate is deliberately not the headless `tools/analyzer-demo` render the hero uses — that one has no chrome, so it photographs the modules and not the program. This only picks up what was left in `packaging/screenshots/`, which is committed — the plate is a Mac's afternoon and a runner cannot retake it. `--only=` does one stage; shooting `desktop` or `tablet` alone re-encodes one half of a pair and is only ever right straight after a run of that script. Each missing stage prints what would make it. |
| `scripts/render-window.mjs` | `npm run window`: the two photographs of the application's own window — the Loudness tab and the Spectrum tab, chrome included — encoded into `public/window/`. Nothing is shot here either; `packaging/app_window_shots.sh` takes them — a launch of the application per tab, because a freeze there is terminal — and this only picks up what it left in `packaging/screenshots/`, committed for the same reason as the pair above. It is the one picture on this site that shows the *program* rather than the meters: the hero still is a web build with no window around it, the thumbnails are one module each, and the signal-path desktop plate is the window mid-publish because that is what its paragraph is about. Four rungs plus the full size, each encoded from the PNG rather than from a larger webp, and a rung wider than the original is skipped rather than upscaled. |
...
| `public/window/*.webp` | The application's window, both tabs — `loudness` and `spectrum` — at 1024, 1440, 1920 and the full capture. Committed output like the rest, because making them needs a release build, the fake DAW, a VST3 and a screen-recording grant. The bare name is the widest, so anything pointing at it still resolves. |
