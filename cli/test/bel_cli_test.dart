...
// enough to pipe.
//
// The exit code matters most. `bel --target` exists so a release pipeline can
...
void main() {
  setUpAll(() {
    _temp = Directory.systemTemp.createTempSync('bel_cli_test');
    _entryPoint = '${Directory.current.path}/bin/bel.dart';
  });
  ...
  group('usage', () {
    ...
    test('--version names the tool and the engine', () {
      ...
      expect(result.exitCode, 0);
      expect(result.stdout, contains('Bel'));
    });
