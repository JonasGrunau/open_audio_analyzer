...
// app does, over the same decoder, so `bel analyze master.wav` and dropping
...
/// What `bel` reports back to the shell.
///
...
const String _version = 'Bel 0.1.0';
...
Future<void> main(List<String> arguments) async {
  ...
  try {
    ...
    stderr.writeln('bel: ${error.message}');
    stderr.writeln();
  ...
  if (args.flag('version')) {
    stdout.writeln('$_version (engine ${BelEngine.versionString})');
    return;
  }
  ...
  if (paths.isEmpty) {
    stderr.writeln('bel: no input files');
    stderr.writeln();
    stderr.writeln(_usage(parser));
    exit(ExitCode.error);
  }
  ...
  if (targetId != null) {
    ...
    if (target == null) {
      stderr.writeln('bel: unknown target "$targetId"');
      stderr.writeln('bel: run `bel --list-targets` to see them');
      exit(ExitCode.error);
    }
  }
  ...
  if (format == ReportFormat.csv && paths.length > 1) {
    stderr.writeln(
      'bel: --format csv writes one timeline, so it takes one file '
      '(${paths.length} given)',
    );
    exit(ExitCode.error);
  }
  ...
  if (outputPath != null && paths.length > 1) {
    stderr.writeln('bel: --output takes one file (${paths.length} given)');
    exit(ExitCode.error);
  }
  ...
  if (parsedInterval == null || parsedInterval <= 0) {
    stderr.writeln(
      'bel: --timeline-interval must be a positive number of '
  ...
  for (final path in paths) {
    ...
    if (!file.existsSync()) {
      stderr.writeln('bel: no such file: $path');
    ...
    try {
      ...
      stderr.writeln('bel: ${error.message}: $path');
      worstExit = ExitCode.error;
      continue;
  ...
  if (outputPath != null) {
    ...
    stderr.writeln('bel: wrote $outputPath');
  } else {
    stdout.write(body);
  }
...
///
/// The two are deliberately separate types: `bel_engine` measures and knows
/// nothing about delivery targets, `bel_core` interprets and knows nothing
...
Usage:  bel [options] <file>...

Measures a file against ITU-R BS.1770-4 and EBU R 128, using the same engine
and the same decoder the Bel app does. WAV, AIFF, RF64, Wave64, FLAC and MP3.
...
Examples:
  bel master.wav
  bel --target streaming-14 master.wav
  bel --format json --timeline master.wav > report.json
  bel --format csv -o loudness.csv master.wav
''';
