 ...
 * reimplement the class — see the table in BelWire.h.
 *
...
BelAudioProcessor::BelAudioProcessor()
    : juce::AudioProcessor(
          BusesProperties()
              .withInput("Input", juce::AudioChannelSet::stereo(), true)
              .withOutput("Output", juce::AudioChannelSet::stereo(), true)) {}

BelAudioProcessor::~BelAudioProcessor() = default;

/* --------------------------------------------------------------------- */
/* Buses                                                                  */
/* --------------------------------------------------------------------- */

bool BelAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const {
  ...
  return in.size() >= 1 && in.size() <= static_cast<int>(BEL_MAX_CHANNELS);
}
...
void BelAudioProcessor::prepareToPlay(double sampleRate, int maximumExpectedSamplesPerBlock) {
...
void BelAudioProcessor::releaseResources() {
  streamer_.release();
}
...
void BelAudioProcessor::captureTransport(int numFrames) noexcept {
  wire::Transport t;
   ...
   * clear — see the long note in BelWire.h on why a missing value must not
...
void BelAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer,
                                     juce::MidiBuffer&) {
...
void BelAudioProcessor::getStateInformation(juce::MemoryBlock& destData) {
  juce::ValueTree state("BelPluginState");
...
void BelAudioProcessor::setStateInformation(const void* data, int sizeInBytes) {
  if (data == nullptr || sizeInBytes <= 0)
    return;

  const auto state = juce::ValueTree::readFromData(data, static_cast<size_t>(sizeInBytes));
  if (!state.isValid() || !state.hasType("BelPluginState"))
    return;
...
juce::AudioProcessorEditor* BelAudioProcessor::createEditor() {
  return new BelPluginEditor(*this);
}

}  // namespace bel

/*
 * The host's entry point. Everything above is in `namespace bel`; this must not
 * be, because JUCE looks it up unqualified.
 */
juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter() {
  return new bel::BelAudioProcessor();
}
