/*
 * BelWire.cpp — serialisers for the Bel wire protocol.
 *
 * SPDX-License-Identifier: AGPL-3.0-or-later
 *
 * The field order below is the normative table in `docs/WIRE.md`, which is
 * itself `bel.h`'s declaration order read top to bottom — **including the
...
constexpr size_t kChannels  = BEL_MAX_CHANNELS;
constexpr size_t kBands     = BEL_SPECTRUM_BANDS;
constexpr size_t kScope     = BEL_SCOPE_POINTS * 2;
constexpr size_t kHistogram = BEL_HISTOGRAM_BINS;
...
void writeSnapshotFrame(std::vector<uint8_t>& out, const bel_snapshot& s,
  ...
  const uint32_t flags =
      dropped != 0 ? (s.flags | uint32_t(BEL_FLAG_OVERRUN)) : s.flags;
...
void writeHelloFrame(std::vector<uint8_t>& out, const char* producerName,
  ...
  w.u32(static_cast<uint32_t>(BEL_MAX_CHANNELS));
  w.u32(static_cast<uint32_t>(BEL_SPECTRUM_BANDS));
  w.u32(static_cast<uint32_t>(BEL_SCOPE_POINTS));
  w.u32(static_cast<uint32_t>(BEL_HISTOGRAM_BINS));
