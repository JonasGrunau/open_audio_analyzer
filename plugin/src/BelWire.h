/*
 * BelWire.h — the Bel wire protocol, producer side.
 *
 ...
 * C++ producer. The Dart implementation in `packages/bel_wire/` is another.
 ...
 *
 * `bel_snapshot` is a plain-old-data struct with no pointers, and on every
 * platform Bel targets it happens to contain no padding at all — `bel.h` puts
 * its two 8-byte members first and everything after is 4-byte, so the layout
 * is dense and `sizeof` is exactly the wire size. A memcpy would be correct
 * today, and it would be measurably faster.
 *
 ...
 * A field added to `bel_snapshot` must be a deliberate protocol-version
 ...
 * is Dart. Keeping this translation unit to libbel and the standard library
...
/* 'B','E','L','W' read as little-endian. Present on every frame rather than
...
inline constexpr uint32_t kMagic = 0x574C4542u;

/* The protocol version, which is deliberately NOT BEL_ABI_VERSION.
 *
 ...
 * `bel_snapshot` untouched — which is exactly what the file-decoding work is —
...
/*
 * If this fires, somebody appended a field to `bel_snapshot`.
 *
 * That is allowed — `bel.h` explicitly invites it — but it is a protocol event,
 ...
 */
static_assert(sizeof(bel_snapshot) == kSnapshotBytes,
              "bel_snapshot changed size. See the comment above: this is a "
 ...
 * without reimplementing the class. The mapping lives in BelPluginProcessor.cpp.
 */
 ...
 * though every platform Bel targets is little-endian today. The cost is a few
 * shifts on a path that runs fifty times a second; the alternative is a
 * protocol whose correctness rests on an unstated assumption, which is the
 * kind of thing that holds for years and then does not.
 */
class ByteWriter {
public:
 ...
 * sets BEL_FLAG_OVERRUN when it is non-zero. It exists because in a plugin the
 ...
 */
void writeSnapshotFrame(std::vector<uint8_t>& out, const bel_snapshot& snapshot,
...
void writeHelloFrame(std::vector<uint8_t>& out, const char* producerName,
                     uint32_t abiVersion = BEL_ABI_VERSION);
