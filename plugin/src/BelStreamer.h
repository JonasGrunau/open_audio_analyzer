/*
 * BelStreamer.h — the thread between the DAW and the Bel app.
 *
 ...
 *
 * `bel_engine_push` is synchronous: it runs the entire DSP graph — K-weighting,
 ...
 *
 * So this follows the same three-tier shape the rest of Bel uses, because it is
 * the same problem:
 *
 ...
 *
 * `engine/src/bel_ring.c` is a working SPSC ring with precisely the right
 ...
 * is engine-internal, not part of `bel.h`, and `bel.h` says plainly that what
 ...
 * directory consumes libbel's public C ABI and nothing else.
 */
 ...
 *
 * Two ports, not one, and the distinction is load-bearing. Bel speaks the same
 * frames in both directions, but the two links dial in opposite directions:
 *
 *   47821  a desktop Bel publishes, a tablet connects in   (remote display)
 *   47822  a plugin publishes, a desktop Bel accepts       (this)
 *
...
private:
  ...
  bel_engine*          engine_   = nullptr;
  const bel_snapshot*  snapshot_ = nullptr;
