 ...
 * `packages/oaa_ui/lib/src/tokens.dart`, and the duplication is not an
...
OaaPluginEditor::OaaPluginEditor(OaaAudioProcessor& processor)
  ...
  configureLabel(hostLabel_, "Open Audio Analyzer app host");
...
void OaaPluginEditor::applyDestination() {
...
void OaaPluginEditor::paint(juce::Graphics& g) {
  ...
  g.setColour(kText);
  g.drawText("Open Audio Analyzer", area.removeFromTop(kRow), juce::Justification::centredLeft);
  ...
  const juce::String linkText = status_.connected
      ? "connected"
      : (status_.everConnected ? "reconnecting" : "waiting for the Open Audio Analyzer app");
