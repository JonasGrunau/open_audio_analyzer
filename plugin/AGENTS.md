...
`libbel`, and streams the resulting snapshot and the host's transport position
to the Bel desktop app over a TCP socket. The app draws.

That division is not a limitation being worked around. Flutter cannot be a
plugin GUI, and the alternative — reimplementing twelve meter modules in C++ so
...
written against the older terms and is out of date on this point). Bel takes the
AGPLv3 option, so anything linked into the plugin binary is AGPL.
...
The practical consequence of AGPL over GPL is its network clause, and since Bel
publishes all its source anyway, that clause asks for nothing new. But it does
mean **nothing in `plugin/` may be moved into `engine/` or `bel_core/`** without
rewriting it, because those are MIT and must stay linkable by people who are not
...
| File | Purpose |
|------|---------|
| `CMakeLists.txt` | JUCE, `libbel`, and the three formats. The pinned JUCE tag is one line; bumping it is a decision. |
| `src/BelWire.h/.cpp` | The wire protocol, producer side. No JUCE include — deliberately, so it is testable without a framework. |
| `src/BelTransportBox.h` | Seqlock handing one transport reading from the audio thread to the streaming thread. |
| `src/BelStreamer.h/.cpp` | Owns the engine. Drains the FIFO, measures, serialises, sends, reconnects. |
| `src/BelPluginProcessor.h/.cpp` | The `AudioProcessor`. Real-time path and playhead capture. |
| `src/BelPluginEditor.h/.cpp` | A status panel. Not a meter, and must not become one. |
...
- **`processBlock` allocates nothing, locks nothing, and calls no syscall.**
  ...
  oversampling, the socket write — is on the streaming thread. `bel_engine_push`
...
- **`docs/WIRE.md` is the protocol's specification; this is one implementation.**
  The Dart implementation in `packages/bel_wire/` is another, written against
...
- **Only the public C ABI.** `bel.h` says that what is not declared there is not
  part of the engine, and that applies here even though `engine/src` is two
  directories away. There is a perfectly good SPSC ring in `bel_ring.h`; the
...
- **Adding a file to `engine/src` means adding it in two places**, here in
  `engine/CMakeLists.txt` and in `packages/bel_engine/hook/build.dart`. There
...
Products land in `plugin/build/BelPlugin_artefacts/Release/`. Nothing is
...
`packages/bel_wire/test/plugin_golden_test.dart`, and the app-side ingest is
